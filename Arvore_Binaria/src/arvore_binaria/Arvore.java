...
public class Arvore<T extends Comparable<T>> 
{
    ...
    public No buscar(T valor, No no)
    {
        if(no == null)
            return null;
        
        else if(no.obterValor().compareTo(valor) == 0)
            return no;
        
        else if(valor.compareTo(valor) == -1)
            return buscar(valor, no.obterNoEsquerdo());
        
        else
            return buscar(valor, no.obterNoDireito());
    }
